...
export default {
    ...
    mounted(){
      this.$nextTick( async () => {
        try {
          await Promise.all([this.GET_DATA(this.apiUrl+'/'+this.$route.params.id), this.GET_DATA('brands'), this.GET_DATA('measures'),this.GET_DATA('categories')]).then((response) => {
            this.model = response[0]
            this.brands = response[1]
            this.measures = response[2]
            this.categories = response[3]
